...
※　通知機能を使いたい場合、iOS(Safari)ではこのアプリ化をしないとはじかれてしまう
...
4. 右上に表示される追加を押すと、ホーム画面にアプリケーションのように表示されます。
...
- カレンダー通知機能（`.ics`ファイル出力でGoogle/Apple/カレンダーに追加）　※ 追記：今後調整予定あるが、実装済み(Webプッシュ機能だけ調整中)
- キャラクターによるナビ機能（固定セリフ or AI連携 もしくは両方） ※ キャラクターはミニキャラを描いて実装予定
- ToDoアプリとしてのUI/UXの向上(具体的にはファイル機能などを実装予定)
- 長期的な目標として、広告化など収益につなげ、サービスの永続化を目指す。
